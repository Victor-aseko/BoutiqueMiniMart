...
const AdminAnalyticsScreen = ({ navigation }) => {
    ...
    const generatePDF = async () => {
        try {
            ...
            // Use itemsPrice for Net Revenue
            const totalRevenue = orders.reduce((acc, o) => acc + (o.itemsPrice || 0), 0);
            ...
            const htmlContent = `
                ...
                <body>
                    <div class="header">
                        ...
                        <div class="subtitle">Date: ${startDate || 'All Time'} to ${endDate || dayjs().format('YYYY-MM-DD')}</div>
                        <div class="subtitle">Generated by: ${adminUser?.name || 'Admin'}</div>
                        ...
                        <tbody>
                            ${orders.map(order => `
                                <tr>
                                    ...
                                    <td>${order.status}</td>
                                    <td>Kshs ${order.itemsPrice.toLocaleString()}</td>
                                </tr>
                            `).join('')}
                        </tbody>
                    </table>
    ...
    const generateExcel = async () => {
        try {
            ...
            let csvContent = `Report Generated by: ${adminUser?.name || 'Admin'}\n`;
            csvContent += `Generated on: ${new Date().toLocaleString()}\n`;
            csvContent += `Date range: ${startDate || 'All Time'} to ${endDate || dayjs().format('YYYY-MM-DD')}\n\n`;
            csvContent += 'Date,Order ID,Customer,Email,Phone,Status,Net Amount (Kshs)\n';
